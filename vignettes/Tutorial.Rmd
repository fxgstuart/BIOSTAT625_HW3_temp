...
### *(3).What can "FitLM" package do?*

We develop an user-friendly R package `FitLM`, which can be seen as an extension of the existing `stats::lm` function. 

Specifically, users can obtain the following results easily by using `FitLM` package:
...
* Obtain the $t$-statistic from $t$-test, and the corresponding $p$-value.
* Obtain the $F$-statistic from partial $F$-test, and the corresponding $p$-value.
* Obtain the $F$-statistic from overall test, and the corresponding $p$-value.
